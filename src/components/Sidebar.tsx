    ...
    <aside className="glass-panel-strong relative hidden border-b border-white/10 md:m-3 md:block md:min-h-[calc(100vh-1.5rem)] md:w-[260px] md:shrink-0 md:overflow-hidden md:rounded-2xl">
      <div className="relative p-4 md:p-5">
        <div className="flex items-center gap-3">
          <div className="grid h-10 w-10 place-items-center rounded-xl border border-slate-600/60 bg-slate-900/70">
          ...
          return (
            <Link
              ...
              className={clsx(
                ...
                active
                  ? "border-slate-500/55 bg-slate-800/70 text-zinc-50 shadow-[inset_3px_0_0_rgba(125,211,252,.55)]"
                  : "border-transparent text-zinc-300/85 hover:border-slate-600/50 hover:bg-slate-800/45 hover:text-white"
              )}
            >
              <Icon className={clsx("relative h-4 w-4 transition duration-200", active ? "text-sky-200" : "group-hover:text-sky-100")} />
              <span>{it.label}</span>
            </Link>
          );
        })}
